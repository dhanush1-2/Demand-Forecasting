...
def show_distributions(df: pd.DataFrame):
    ...
    selected_feature = st.selectbox("Select Feature", numeric_cols, index=numeric_cols.index("target_demand") if "target_demand" in numeric_cols else 0)
...
def show_correlations(df: pd.DataFrame):
    ...
    important_cols = [
        "target_demand", "historical_sales", "price", "promotion_flag",
        "holiday_flag", "economic_index", "day_of_week", "month"
    ]
    ...
    if len(selected_cols) >= 2:
        ...
        if "target_demand" in selected_cols:
            ...
            target_corr = corr_matrix["target_demand"].drop("target_demand").sort_values(key=abs, ascending=False)
